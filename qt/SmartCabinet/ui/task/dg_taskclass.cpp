#include "dg_taskclass.h"
#include "ui_dg_taskclass.h"
...
Dg_TaskClass::Dg_TaskClass(QWidget *parent) :
    QDialog(parent),
...
void Dg_TaskClass::on_pB_exec_clicked()
{
...
//申请
void Dg_TaskClass::on_pB_delete_clicked()
{
    //将待选框试剂加入等待执行框
...
//删除按钮
void Dg_TaskClass::on_pB_apply_clicked()
{
    //将待选框试剂加入等待执行框
...
void Dg_TaskClass::on_pB_search_clicked()
{
    QString searchGoal = "";
...
void Dg_TaskClass::ShowSQL()
{
    ...
    commonModel->LoadSQL(ui->TW_Sea, table_sea, s_hearders);
}



//升序
void Dg_TaskClass::SortAscend()
{
    ui->TW_Exec->sortItems(0, Qt::AscendingOrder);
    ui->TW_Sea->sortItems(0, Qt::AscendingOrder);
}

void Dg_TaskClass::mousePressEvent(QMouseEvent *event)
{
...
bool Dg_TaskClass::GetContent(QTableWidget *source, QString name)
{
...
//T_AgentiaExecute  执行列表
//T_AgentiaSaving 试剂列表
void Dg_TaskClass::Write_Sql(QMap<int, QString> map, QString &name)
{
    QSqlQuery query;
...
void Dg_TaskClass::ShowExecuteWindow()
{
...
void Dg_TaskClass::DownLoad_TaskList()
{
//    QSqlQuery query;
...
void Dg_TaskClass::SetTitle(QString name)
{
    ui->LB_title->setText(QStringLiteral("%1").arg(name));
}


void Dg_TaskClass::Del(QTableWidget *source)
{
...
void Dg_TaskClass::Add(QTableWidget *goal, QTableWidget *source)
{
    QList<QTableWidgetItem *> array = source->selectedItems();
    int count = array.count();
    int old = -1;
