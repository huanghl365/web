...
//     query.exec(QString("insert into T_AgentiaReturnSave values (1,'1酒精','未摆放','600ml','300ml',1,2,'2010.7.7',1,123,2)"));
    ...
    query.exec(QString("create table T_AgentiaCheck (id int primary key,[agentiaName] varchar,[judgeAttitude] varchar,\
                      [bottleCapacity] varchar,[dose] varchar,[expireDate] varchar,[drawerNo] int,[positionNo] int,\
                      [attribute] int,[agentiaId] int,[positionId] int)"));
    //点验 操作数据表

    query.exec(QString("create table T_AgentiaReplace (id int primary key,[agentiaName] varchar,[judgeAttitude] varchar,\
                      [bottleCapacity] varchar,[dose] varchar,[expireDate] varchar,[drawerNo] int,[positionNo] int,\
                      [attribute] int,[agentiaId] int,[positionId] int)"));
   //替换 操作数据表


    query.exec(QString("insert into T_AgentiaReplace values (1,'1酒精','未摆放','100ml','400ml','2010.7.7',1,1,1,123,2)"));
